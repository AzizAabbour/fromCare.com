...
export default function Register() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-20 relative">
      <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at 70% 50%, rgba(139,92,246,0.03) 0%, transparent 50%), radial-gradient(ellipse at 30% 70%, rgba(0,240,255,0.03) 0%, transparent 50%)' }} />
      ...
      <motion.div initial={{ opacity: 0, y: 30, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.6 }} className="w-full max-w-md relative z-10">
        <div className="glass-card-strong p-8 md:p-10">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold mb-2" style={{ fontFamily: 'var(--font-primary)' }}>Create Account</h1>
            <p className="text-white/40 text-sm">Join the future of mobility</p>
          </div>

          <div className="space-y-4 mb-6">
            <div className="relative">
              <FiUser className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30" />
              <input type="text" placeholder="Full Name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full pl-11 pr-4 py-3.5 rounded-xl text-sm text-white placeholder-white/30 outline-none focus:border-cyan-500/30 transition-colors" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }} />
            </div>
            <div className="relative">
            ...
            <div className="relative">
              ...
              <input type="tel" placeholder="Phone Number" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="w-full pl-11 pr-4 py-3.5 rounded-xl text-sm text-white placeholder-white/30 outline-none focus:border-cyan-500/30 transition-colors" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }} />
            </div>
            <div className="relative">
            ...
            <div className="relative">
              <FiLock className="absolute left-4 top-1/2 -translate-y-1/2 text-white/30" />
              <input type="password" placeholder="Confirm Password" value={form.confirm} onChange={(e) => setForm({ ...form, confirm: e.target.value })} className="w-full pl-11 pr-4 py-3.5 rounded-xl text-sm text-white placeholder-white/30 outline-none focus:border-cyan-500/30 transition-colors" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)' }} />
            </div>
          </div>

          <label className="flex items-start gap-2 text-white/40 text-xs mb-6">
            <input type="checkbox" className="accent-cyan-400 mt-0.5" />
            <span>I agree to the <a href="#" className="text-cyan-400">Terms of Service</a> and <a href="#" className="text-cyan-400">Privacy Policy</a></span>
          </label>

          <button className="glow-btn-solid w-full !py-3.5 mb-6">Create Account <FiArrowRight /></button>

          <div className="relative mb-6"><div className="absolute inset-0 flex items-center"><div className="w-full" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }} /></div><div className="relative flex justify-center"><span className="px-4 text-xs text-white/30" style={{ background: 'var(--bg-secondary)' }}>or</span></div></div>
          ...
          <p className="text-center text-sm text-white/40">Already have an account? <Link to="/login" className="text-cyan-400 hover:text-cyan-300 font-medium">Sign In</Link></p>
        </div>
      </motion.div>
    </div>
  );
